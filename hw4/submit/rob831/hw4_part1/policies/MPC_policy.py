...
class MPCPolicy(BasePolicy):
    ...
    def sample_action_sequences(self, num_sequences, horizon, obs=None):
        if self.sample_strategy == 'random' \
            ...
            for i in range(self.cem_iterations):
                ...
                sampled_sequences = np.where(acs<self.high, acs, self.high)
                elite_indices = np.argsort(self.evaluate_candidate_sequences(sampled_sequences, obs))[-self.cem_num_elites:]
                elites = sampled_sequences[elite_indices]
                mean = self.cem_alpha * np.mean(elites) + (1-self.cem_alpha) * mean
    ...
    def evaluate_candidate_sequences(self, candidate_action_sequences, obs):
        ...
        predicted_rewards_cumulative = []
        for model in self.dyn_models: 
            predicted_rewards = self.calculate_sum_of_rewards(obs, candidate_action_sequences, model)
            predicted_rewards_cumulative.append(predicted_rewards)

        return np.mean(predicted_rewards_cumulative, axis=0)
    ...
    def calculate_sum_of_rewards(self, obs, candidate_action_sequences, model):
        ...
        for i in range(self.horizon):
            ...
            next_obs_batch = model.get_prediction(obs_batch, action_batch, self.data_statistics)
            rewards = self.env.get_reward(obs_batch, action_batch)
